...
> [!NOTE]
> This ADR records the upstream `mattpocock/skills` distribution decision. This fork retains a Claude plugin manifest for its promoted set, but its supported personal installation is the symlink workflow in [the install block](../install-block.md). References below to the official marketplace apply to the upstream plugin.
