...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue under the personal naming and module-structure standards, uses Ponytail when available, drives **`/tdd`** internally (one red-green slice at a time), then closes out with **`/code-review`** before committing. The review keeps Standards and Spec as its two core axes and adds a separate Ponytail perspective when `ponytail-review` is available. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
